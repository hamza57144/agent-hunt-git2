...
namespace CoverShooter
{
    ...
    /// <summary>
    /// Direction of character facing related to some direction.
    /// </summary>
    ...
    public class CharacterMotor : MonoBehaviour
    {
        ...
        /// <summary>
        /// Position the head is currently looking at.
        /// </summary>
        public Vector3 HeadTarget
        {
            get { return _currentHeadTarget; }
        }
        ...
        /// </summary>
        public Vector3 AimTargetForward
        {
            get
            {
        ...
        public Vector3 HeadForward
        {
            get
            {
                var vec = HeadTarget - transform.position;
                vec.y = 0;
        ...
        /// <summary>
        /// Shift in degrees of the weapon affected by recoil.
        /// </summary>
        public float Bloom
        {
            get
            {
                ...
                if (gun != null)
                    return gun.Recoil.BaseBloom + (gun.Recoil.MaxBloom - gun.Recoil.BaseBloom) * _bloomLerp;
                else
                    return 0;
            }
        }
      ...
      /*  public bool IsObjectVisibleToCamera( )
        ...
        /// <summary>
        /// Current or last movement direction.
        /// </summary>
        ...
        /// <summary>
        /// Multiplier for gun bloom properties.
        /// </summary>
        [Tooltip("Multiplier for gun bloom properties.")]
        ...
        /// <summary>
        /// Don't deal melee damage to friends.
        /// </summary>
        [Tooltip("Don't deal melee damage to friends.")]
        public bool FriendlyMeleeDamage = false;
        ...
        private float _bloomLerp;
        private bool _inputBloom;
        ...
        private float _meleeVerticalAngle;
        ...
        public void InputEndRoll()
        {
            ...
            if (_potentialCover == null && _cover.In)
                InputLeaveCover();
        }
        ...
        /// <summary>
        /// Inputs a command to take cover. Ignore all human-related niceness.
        /// </summary>
        public void InputForceTakeCover()
        {
        ...
        {
            //if (gameObject.layer != Layers.Character)
            //Debug.LogError("Please set character layer to Character");
        }
        ...
        private void LateUpdate()
        {
            ...
            if (isPlayer && ActiveWeapon.Gun != null)
            {
                if (ActiveWeapon.Gun.myScope == null)
                {
                    hasScope = false;
                }
                if (ActiveWeapon.Gun.myScope != null)
            ...
            if (IsAlive)
            {
                ...
                if (_isMoving && _currentMovement.Direction.magnitude > 0.5f)
                    _moveDirection = _currentMovement.Direction;
                ...
                if (EquippedWeapon.Gun != null && !IsInCover && !EquippedWeapon.PreventArmLowering && _isInCameraMode)
                {
            ...
            {
                if (_inputBloom)
                    _bloomLerp = Math.Min(1, Math.Max(0, _bloomLerp + Time.deltaTime / gun.Recoil.BloomTime));
                else
                    _bloomLerp = Math.Min(1, Math.Max(0, _bloomLerp - Time.deltaTime / gun.Recoil.BloomRecoveryTime));
            }
            else
                _bloomLerp = 0;
        ...
        private void updateRolling()
        {
            ...
            if (_isRolling && _potentialCover == null && _cover.In)
                InputLeaveCover();
        }
        ...
        private void moveAndRotate()
        {
            ...
            else
            {
                ...
                if (_cover.In && _cover.Main.IsFront(Util.HorizontalAngle(_currentMovement.Direction)))
                {
        ...
        private void updateCover()
        {
            ...
            if (_cover.In && _isGrounded && (_inputMovement.IsMoving || !IsAiming))
        ...
        private void updateCommon()
        {
            ...
            else if (_wantsToImmediatelyUpdatePotentialCover ||
        ...
        {
            var ignore = _isRolling;

            if (_currentMovement.IsMoving && !IsAiming)
                if (_cover.Main.IsBack(Util.HorizontalAngle(_currentMovement.Direction), 5))
                    ignore = true;
        ...
        private void updateIK()
        {
            ...
            if (IK.Enabled)
            {
                if (_isPerformingMelee)
                {
                    ...
                    var angle = Util.VerticalAngle(_meleeTarget - chest.transform.position);
                    angle = Mathf.Clamp(angle, -30, 20);

                    Util.LerpAngle(ref _meleeVerticalAngle, angle, 6);
                }
                else
                    Util.LerpAngle(ref _meleeVerticalAngle, 0, 6);

                if (_meleeVerticalAngle < -1 || _meleeVerticalAngle > 1)
                {
                    var spine = _animator.GetBoneTransform(HumanBodyBones.Spine);

                    var forward = _meleeTarget - transform.position;
                    forward.y = 0;
                    forward.Normalize();

                    var right = Vector3.Cross(Vector3.up, forward);

                    spine.transform.rotation = Quaternion.AngleAxis(_meleeVerticalAngle, right) * spine.transform.rotation;
                }
            }
            else
            {
                _meleeVerticalAngle = 0;
            }
            ...
            if (IK.Enabled)
            {
                ...
                config.BodyTarget = _hasBodyTarget ? _bodyTarget : transform.position + transform.forward * 100;
                config.AimTarget = _hasAimTarget ? _aimTarget : transform.position + transform.forward * 100;
                config.IsPrecise = _isAimingPrecisely;

                _ik.Update(config, out _currentHeadTarget);
            }
            else
            {
                _currentHeadTarget = transform.position + transform.forward * 100;
                _ik.Skip();
            }
        ...
        private void findGroundAndSlope()
        {
            _isOnSlope = false;
            _isGrounded = false;
            ...
            {
                var maxShift = highest - _vertical.Offset;
