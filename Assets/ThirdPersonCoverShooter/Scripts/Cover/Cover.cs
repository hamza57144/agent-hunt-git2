...
namespace CoverShooter
{
    ...
    {
        /// <summary>
        /// BaseActor that has taken a cover.
        /// </summary>
    ...
    public class Cover : MonoBehaviour
    {
        ...
        private Dictionary<BaseActor, CoverUser> _userMap = new Dictionary<BaseActor, CoverUser>();
        ...
        /// </summary>
        public void RegisterUser(BaseActor actor, Vector3 position)
        {
            CoverUser user;
            user.Actor = actor;
        ...
        /// </summary>
        public void UnregisterUser(BaseActor actor)
        {
        ...
        private void Update()
        {
            foreach (var user in _userMap.Values)
                Debug.DrawLine(user.Position, user.Actor.transform.position, Color.cyan);
        }
        ...
        public bool IsInFront(Vector3 observer, bool isOld = false)
        {
        ...
        private void checkBounds()
        {
            ...
            if (!_hasBounds)
            {
                _min = collider.bounds.min;
                _max = collider.bounds.max;
                _hasBounds = Application.isPlaying;
            }
        }
