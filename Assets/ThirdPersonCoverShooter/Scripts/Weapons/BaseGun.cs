...
namespace CoverShooter
{
    ...
    public abstract class BaseGun : MonoBehaviour
    {
        ...
        /// <summary>
        /// Number of bullets left in the gun plus in the inventory.
        /// </summary>
        public abstract int TotalBulletsLeft { get; }
        ...
        protected virtual void Frame()
        {
            ...
            // Notify character if the trigger is pressed.
            {
            ...
            // Directly fire bullets without delay.
            if (_isFiringOnNextUpdate && _isAllowed)
            {
                ...
                for (int i = 0; i < BulletsPerShot; i++)
                {
                    ...
                    // Instant hit without delay
                    if (fire(0f, !ConsumeSingleBulletPerShot))
                        hasFired = true;
                }
        ...
        public RaycastHit Raycast(Vector3 origin, Vector3 direction, out bool isFriend, bool friendCheck)
        {
            ...
            isFriend = false;
            var count = Physics.RaycastNonAlloc(origin, direction, _hits, Distance, Layers.Geometry | Layers.Character, QueryTriggerInteraction.Collide);
        ...
        private bool fire(float delay, bool consume)
        {
            ...
            if (Character != null)
            {
                var right = Util.HorizontalVector(Util.HorizontalAngle(direction) + 90);

                var h = UnityEngine.Random.Range(-1f, 1f);
                var v = UnityEngine.Random.Range(-1f, 1f);
                ...
                var bloom = UnityEngine.Random.Range(0, Character.Bloom);

                var recoilRotation = Quaternion.AngleAxis(dir.x * bloom, right) * Quaternion.AngleAxis(dir.y * bloom, Vector3.up);
