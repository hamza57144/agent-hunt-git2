    ...
    public class AIAlerts : MonoBehaviour
    {
        ...
        public void OnScared()
        {
            if (_actor.IsAlive && Fear > float.Epsilon)
                Alerts.Broadcast(transform.position, Fear, AlertType.Chat, _actor, true);
        }
        ...
        public void OnBackupCall()
        {
            if (_actor.IsAlive && BackupCall > float.Epsilon)
                Alerts.Broadcast(transform.position, BackupCall, AlertType.Chat, _actor, true);
        }
        ...
        public void OnCopCall()
        {
            if (_actor.IsAlive && CopCall > float.Epsilon)
                Alerts.Broadcast(transform.position, CopCall, AlertType.Chat, _actor, true);
        }
        ...
        public void OnCoverSwitch()
        {
            if (_actor.IsAlive && CoverSwitch > float.Epsilon)
                Alerts.Broadcast(transform.position, CoverSwitch, AlertType.Chat, _actor, true);
        }
        ...
        public void OnAssaultStart()
        {
            if (_actor.IsAlive && Assault > float.Epsilon)
                Alerts.Broadcast(transform.position, Assault, AlertType.Chat, _actor, true);
        }
    }
}
