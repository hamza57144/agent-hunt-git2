    ...
    public sealed class Util
    {
        ...
        public static bool IsFree(GameObject gameObject, Vector3 origin, Vector3 direction, float distance, bool coverMeansFree, bool actorMeansFree)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (!Hits[i].collider.isTrigger && !((Hits[i].collider as TerrainCollider) is null) && !InHiearchyOf(Hits[i].collider.gameObject, gameObject))
                    isFree = false;
            }
        ...
        {
            if (ControlFreak2.CF2Input.touchCount == 2)
            {
                // Store both touches.
                ControlFreak2.InputRig.Touch touchZero = ControlFreak2.CF2Input.GetTouch(0);
                ControlFreak2.InputRig.Touch touchOne = ControlFreak2.CF2Input.GetTouch(1);
        ...
        public static float HorizontalDistance(Vector3 a, Vector3 b, float threshold = 30)
        {
            var vector = a - b;
            var angle = vector.y > 0 ? VerticalAngle(vector) : VerticalAngle(-vector);
